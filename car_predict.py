...
def model(filename):
    ...
    #원-핫 한코딩 후에 훈련데이터랑 테스트 데이터 열(칼럼)체크
    #훈련 데이터 목록
    train_cols = set(df_train_word.columns) #집합,{},키 값없는 딕셔너리,합지합/교집합/차집합
    ...
    # 평가 - score(테스트독립변수, 테스트종속변수)
    print('R2 score :',RForest_model.score(X_train,y_train))

    # 평가 - RMSE
    X_predict = RForest_model.predict(X_val)
    print(' RandomForestRegressorn:', root_mean_squared_error(y_val,X_predict))   #예측값
